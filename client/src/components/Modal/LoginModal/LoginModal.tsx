...
const LoginModal: FC = () => {
  ...
  const [modalType, setModalType] = useState<"로그인" | "회원가입">("로그인");
  const [email, setEmail, changeEmail] = useInput("");
  const [isSendEmail, setIsSendEmail] = useState<boolean>(false);

  const changeModalType = useCallback(() => {
    setModalType((prev) => (prev === "로그인" ? "회원가입" : "로그인"));
  }, []);
      ...
      <S.Background>
        <S.Modal>
          ...
          <S.ModalRight>
            ...
            <S.FormWrap>
              <S.FormContent>
                <S.ModalTitle>{modalType}</S.ModalTitle>
                <S.SubContentWrap>
                  <S.SubTitle>이메일로 {modalType}</S.SubTitle>
                  <S.Form onSubmit={formHandler}>
                    {isSendEmail ? (
                      <S.SuccessMessage>메일을 전송했습니다</S.SuccessMessage>
                    ) : (
                      <>
                        <input
                ...
                <S.SubContentWrap>
                  <S.SubTitle>소셜 계정으로 {modalType}</S.SubTitle>
                  <S.SnsWrap>
                    <S.SnsIcon to="#" backgroundColor="rgb(39, 46, 51)">
                      <svg
                        width="20"
                        height="20"
                        fill="currentColor"
                    ...
                    <S.SnsIcon to="#" backgroundColor="white">
                      <svg
                        width="20"
                        height="20"
                        fill="none"
                    ...
                    <S.SnsIcon to="#" backgroundColor="rgb(59, 89, 152)">
                      <svg
                        width="20"
                        height="20"
                        fill="none"
              ...
              <S.HelperRegister>
                ...
                <span onClick={changeModalType} className="link">
                  {modalType === "로그인" ? "회원가입" : "로그인"}
                </span>
              </S.HelperRegister>
            </S.FormWrap>
          </S.ModalRight>
